...
const pressButton = (buttonKey, status) => {
  ...
  // 判断元素属性是否为 data-state="unchecked"
  const current = button.getAttribute('data-state') === 'checked';
  if (current !== status) {
    button.click();
  } else {
    console.log('状态一致')
...
const messageHandlers = {
  ...
  NEW_CHAT: (data) => {
    const newBtns = [
      document.querySelector('span.text-brand-main'),
      document.querySelector('.bg-grey-light-main-bg button.cursor-pointer')
    ]
